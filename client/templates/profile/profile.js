...
if(Meteor.isCordova){
	...
	Template.profile.events({
		/*'scrollstart .contentBackground' : function(e){//scrollun en ustte mi en altta mi oldugu kontrolu
			
			if($(e.currentTarget).scrollTop() + $(e.currentTarget).innerHeight() == $(e.currentTarget)[0].scrollHeight){//eger scrollun suanda ki konumu + icerdeki divin toplami sayfanin tamaminin buyuklugune esitse $(e.currentTarget)[0].scrollHeight bottom!
				$('.roomAndroid:last').addClass("animated bounce");
				
				setTimeout(function(){//animate classini 2 saniye sonra kaldir
                    $('.roomAndroid:last').removeClass("animated bounce");
                }, 2000);
		...
		"swiperight #listAndroid" : function(e){
			e.preventDefault();
			$('.messageList').addClass("fadeOut");
			Router.go('/contacts');
			
		},
		
		"swipeleft #listAndroid" : function(e){
			e.preventDefault();
			$('.messageList').addClass("fadeOut");
			Router.go('/settings');	
		},
		...
		"taphold .roomAndroid" : function(e){
			...
			var addClick = $('#listAndroid').children();//listandroidin tum elemanlarini degiskene atiyoruz
			...
			var img = $('#' + room.id + ' img');//secilen odanin resmi selector
			console.log(img.attr('src'));//secilen resmin src attributeu
			
			$('#' + room.id + ' img').addClass("animated rollOut");//basili tutulan odanin resmi rollOut olup kayboluyor
			
			setTimeout(function (){// daha sonra img tagi ekleyebilmemiz icin eski img i kaldirmamiz gerekiyor ve rollOut animasyonunun delay suresi 1 saniye oldugundan dolayi setTimeOut ile remove fonksiyonunu calistiriyoruz 
				$('#' + room.id + ' img').remove();
				
			}, 650);
			
			setTimeout(function (){
				$('#' + room.id + ' h2').before('<img src="backgrounds/messageBackground2.jpg" class="profileImg animated rollIn">');//resim kaybolduktan sonra yeni resmi eklemek icin 
			}, 700);
