...
if(Meteor.isCordova || Meteor.isClient){
        ...
        if(localStorage.getItem('full') === 'true'){//eger kullanici karsi tarafa gondermek icin sectigi resmi onaylar ise true olur
            ...
            if(Platform.isAndroid()){//android ise
                ...
                if(localStorage.getItem('photoHeight') / localStorage.getItem('photoWidth') < 0.70){// height/width orani 0.70 ten kucuk ise
                    $('.insideAndroid').append('<div class="bubblePhoto me fromMe" id='+ Random.id() +' style="height:150px; width:75px;"><img src="' + localStorage.getItem('selectedPhoto') + '" class="img"></img></div>');
                ...
                if((height / width) < 0.64){//height/width 0.64 ten kucuk ise
                    ...
                    $('.inside').append('<div class="bubble64 me" ><div class="bubblePhoto64 me fromMePhoto" id='+ Random.id() +' style="background-image : url(' + localStorage.getItem('selectedPhoto') + '); "></div></div>');
                    localStorage.setItem('full', 'false');
                }
                else if((height / width) < 0.66){//height/width 0.66 ten kucuk ise
                    console.log("lower than 0.66");
                    $('.inside').append('<div class="bubble66 me""><div class="bubblePhoto me fromMePhoto" id='+ Random.id() +' style="height:100px; width:150px; background-image : url(' + localStorage.getItem('selectedPhoto') + '); background-size: cover; zoom:110%; border:0.5px solid #b9f6ca; background-clip : content-box;"></div></div>');
                    localStorage.setItem('full', 'false');

                }
                else if((height / width) < 0.70){//height/width 0.70 ten kucuk ise
                    console.log("lower than 0.70");
                    $('.inside').append('<div class="bubble70 me" style="max-width: 69%;"><div class="bubblePhoto me fromMePhoto" id='+ Random.id() +' style="height:125px; width:250px; background-image : url(' + localStorage.getItem('selectedPhoto') + '); background-size: cover; border:0.5px solid #b9f6ca"></div></div>');
                    localStorage.setItem('full', 'false');
                }
                else{//height/width orani yuksek oranda ise
                    console.log("else");
                    $('.inside').append('<div class="bubbleHigh me" style="max-width:77%;"><div class="bubblePhoto me fromMePhoto" id='+ Random.id() +' style="height:200px; width:275px; background-image : url(' + localStorage.getItem('selectedPhoto') + '); background-size: cover; border:0.5px solid #b9f6ca;zoom: 103%;"></div></div>');
                    localStorage.setItem('full', 'false');
                }
            }
    ...
    window.addEventListener('native.keyboardshow', function(event){//klavye acildigi zaman android height dusur ve en son mesaja scroll yap
        if(Platform.isAndroid()){
            $('.insideAndroid').css('height', '93%');
            $('.bubble:last').get(0).scrollIntoView();
            ...
            $('.bubble:last').get(0).scrollIntoView();//en son mesaja odaklan
        }
    ...
    window.addEventListener('native.keyboardhide', function(event){//klavye kapandiginda eger textarea da birsey yoksa height oranlarini dusur
        if(Platform.isAndroid()){
            $('.insideAndroid').css('height', '100%'); //android icin kucultulen div buyut
    ...
    Template.messageScreen.events({
        ...
        "click #sendAndroid" : function(e){
            ...
            var insideAndroid = $(".insideAndroid");
        ...
        "click #sendiOS": function(e)
        {
            ...
            document.getElementById("message").value="";/* Textareanin icini bosalt */
            $('.bubble:last').get(0).scrollIntoView(); /* iOS icin her send e basildigi zaman scrollu en alta kaydirma */
